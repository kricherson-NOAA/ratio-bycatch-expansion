...
do_ratio_est_multi <- function(ob_dat, ft_dat, strata, expfactor, bycatchspp, bycatchunit, management_groups = TRUE)
{
  require(dplyr)
  require(tidyr)
  require(rlang)
  ...
  byc_ratios <- do_ratio_multi(ob_dat = ob_dat, strata = strata, expfactor = expfactor, bycatchspp = bycatchspp, bycatchunit = bycatchunit, management_groups = management_groups)
